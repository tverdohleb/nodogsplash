...
http_nodogsplash_serve_splash(request *r, t_auth_target *authtarget, t_client *client, char *error_msg) {
    ...
    httpdAddVariable(r, "uamip", authtarget->ip);
    ...
    free(tmpstr);
    safe_asprintf(&tmpstr, "%d", authtarget->port);
    httpdAddVariable(r, "uamport", tmpstr);
    free(tmpstr);
...
http_nodogsplash_make_authtarget(char* token, char* redir) {
    ...
    safe_asprintf(&(authtarget->authtarget), "%s?uamip=%s&uamport=%s&nasid=%s&nashash=%s&userurl=%s&token=%s",
            config->authURL,
            authtarget->ip,
            authtarget->port,
            config->nas_id,
